...
try {
    ...
    FOR EACH ROW
    BEGIN
        INSERT INTO packages_log (package_id, old_package_name, old_description, old_price, old_items)
        VALUES (OLD.id, OLD.package_name, OLD.description, OLD.price, OLD.items);
    END;
    ";
...
$stmt1 = $db->prepare("SELECT * FROM packages WHERE id = :id");
$stmt1->bindParam(':id', $package_id);
$stmt1->execute();
                ...
                <div class="card-body">
                    ...
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="package_name" name="package_name" value="<?php echo $datas['package_name']?>" required>
                        </div>

                        <!-- Package Description -->
                        <div class="mb-3">
                            <label for="package_description" class="form-label">Package Description</label>
                            <textarea class="form-control" id="package_description" name="package_description" rows="3" required><?php echo $datas['description']; ?>
                            </textarea>
                        </div>

                        <div class="mb-3">
                            <label for="package_items" class="form-label">Items</label>
                            <textarea class="form-control" id="package_items" name="package_items" rows="3" required><?php echo $datas['items']; ?>
                            </textarea>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="package_price" name="package_price" value="<?php echo $datas['price']?>"" required>
                        </div>
                        <?php } ?>
