...
if (isset($_POST["next"])) {
    ...
    if ($numRows > 0) {
        ...
        $stmtPending = $db->prepare("SELECT id FROM pending_orders WHERE user_id = $user_id ORDER BY created_at ASC LIMIT 1");
        $stmtPending->execute();
        $pendingID = $stmtPending->fetchAssoc(PDO::FETCH_ASSOC);
        ...
        $sqlInsertBooking = "INSERT INTO customer_detail (date_of_delivery, time_of_delivery, mode_of_payment, reference_number, pending_order_id, user_id) 
                             VALUES (:date_of_delivery, :time_of_delivery, :mode_of_payment, :reference_number, :pending, :user_id)";
        ...
        $stmt1->bindParam(':pending_order_id', $pendingID);
