...
// Add Logs
$file = 'logs.txt';
$log = date('Y-m-d H:i:s') . ' - User ID: ' . $user_id . ' - Action: Booking attempt' . PHP_EOL;
...
if (!empty($newAddress)) {
    ...
    if (!$stmtUpdateAddress->execute()) {
        $log = date('Y-m-d H:i:s') . ' - User ID: ' . $user_id . ' - Action: Failed to update address' . PHP_EOL;
        file_put_contents($file, $log, FILE_APPEND);
        echo "<script>alert('Failed to update the address. Please try again.');</script>";
        exit;
    } else {
        $log = date('Y-m-d H:i:s') . ' - User ID: ' . $user_id . ' - Action: Address updated successfully' . PHP_EOL;
        file_put_contents($file, $log, FILE_APPEND);
    }
}
...
if ($numRows > 0) {
    $log = date('Y-m-d H:i:s') . ' - User ID: ' . $user_id . ' - Action: Booking failed - Date and time not available' . PHP_EOL;
    file_put_contents($file, $log, FILE_APPEND);
    echo "<script>alert('The selected date and time are not available.');</script>";
} else {
    // Insert booking information into customer_detail using PDO
    ...
    if ($stmt1->execute()) {
        $bookingID = $db->lastInsertId();  // Get the last inserted booking ID
        $log = date('Y-m-d H:i:s') . ' - User ID: ' . $user_id . ' - Action: Booking successful - Booking ID: ' . $bookingID . PHP_EOL;
        file_put_contents($file, $log, FILE_APPEND);
        ...
        $log = date('Y-m-d H:i:s') . ' - User ID: ' . $user_id . ' - Action: Booking failed - Database error' . PHP_EOL;
        file_put_contents($file, $log, FILE_APPEND);
        echo "<script>alert('An error occurred while processing your booking. Please try again later.');</script    <div class="container text-center">
      <div class="mb-3">
