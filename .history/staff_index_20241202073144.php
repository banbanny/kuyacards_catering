...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <body>
    ...
    <a href="logout_admin.php" class="nav-link text-danger">Logout</a>
</nav>
        ...
        <div class="container">
          ...
          foreach ($package_results as $package) {
            echo '<tr>
                    ...
                    <a href="delete_package.php?delete_id='.$package['id'].'" class="btn btn-danger btn-sm" onclick="return confirm(\'Are you sure you want to delete this package?\')">Delete</a><a href="update_package.php?update_id='.$package['id'].'" class="btn btn-danger btn-sm" onclick="return confirm(\'Are you sure you want to update this package?\')">Add</a>
  ...
  <table class="table table-bordered table-striped">
            ...
            <tbody>
              <?php foreach($orders as $order) {?>
                <tr>
                  <td><?php echo $order['pending_order_id'] ?></td>
                  <td><?php echo $order['full_name'] ?></td>
                  <td><?php echo $order['total'] ?></td>
                  <td><?php echo $order['date_of_delivery'] ?></td>
                  <td><?php echo $order['time_of_delivery'] ?></td>
                  <td>
                      <!-- Add a dropdown for status -->
                      <select class="status-dropdown" data-order-id="<?php echo $order['pending_order_id']; ?>">
                          <option value="pending" <?php if ($order['status'] == 'Pending') echo 'selected'; ?>>Pending</option>
                          <option value="paid" <?php if ($order['status'] == 'Paid') echo 'selected'; ?>>Done</option>
                      </select>
                  </td>
                  <td><?php if($order['mode_of_payment'] == "") {echo "Cash on Delivery";} else {echo $order['mode_of_payment'];} ?></td>
                  <td><?php echo $order['address'] ?></td>
                  <td>
                      <!-- Dropdown for Delivery Status -->
                      <select class="delivery-status-dropdown" data-order-id="<?php echo $order['pending_order_id']; ?>">
                          ...
                          <option value="cancelled" <?php if ($order['delivery_status'] == 'Cancelled') echo 'selected'; ?>>Cancelled</option>
                      </select>
                  </td>
                </tr>
              <?php } ?>
...
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    ...
    <!-- Add JavaScript for auto-update -->
    <script>
    $(document).ready(function() {
        // When a dropdown value is changed
        $(".status-dropdown").change(function() {
            ...
            $.ajax({
                url: "update_order_status.php", // Your PHP file that will handle the update
                method: "POST",
                data: {
                    order_id: orderId,
                    status: status
        ...
        $(".delivery-status-dropdown").change(function() {
            ...
            // AJAX request to update the delivery status in the database
            $.ajax({
                url: "update_delivery_status.php", // New PHP file to handle the delivery status update
                method: "POST",
                data: {
                    order_id: orderId,
