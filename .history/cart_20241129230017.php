...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update'])) {
    ...
    if (isset($_SESSION['cart'][$index])) {
        ...
        if ($newQuantity > 0) {
            ...
            unset($_SESSION['cart'][$index]); // Remove item if quantity is less than 1
        }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['proceed_to_payment'])) {
    ...
    foreach ($_SESSION['cart'] as $item) {
        ...
        if (!isset($item['quantity']) || !is_numeric($item['quantity']) || $item['quantity'] <= 0) {
            $item['quantity'] = 1; // Default to 1 if quantity is not valid
        }
        ...
        $stmt->execute([
            ...
            ':user_id' => $user_id // Using the logged-in user's ID
        ]);
    }
    ...
    echo "<script>alert('Your order has been placed successfully!'); window.location.href = 'thank_you.php';</script>";
}
?>
