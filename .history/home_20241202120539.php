...
echo "User is logged in: " . $_SESSION['email']; // For debugging

// Function to handle adding items to the cart session
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['package_name'])) {
    $packageName = $_POST['package_name'];
    $packagePrice = $_POST['price'];

    // Initialize the cart session if it's not already set
    if (!isset($_SESSION['cart'])) {
        $_SESSION['cart'] = [];
    }

    // Add the package to the session cart
...
<body>
    ...
    <section id="pricing" class="pricing-container">
        ...
        <div class="flex-container">
            <!-- Package A -->
             <?php foreach($package_results as $results) {?>
            <div class="flex-item">
                <ul class="package">
                    <li class="header"><?php echo $results['package_name'] ?></li>
                    <li class="gray">&#8369;<?php echo $results['price'] ?><br><span class="month"><?php echo $results['description'] ?></span></li>
                    <?php echo $results['items'] ?>
                    <form method="POST">
                        <input type="hidden" name="package_name" value="<?php echo $results['package_name'] ?>">
