...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['item_name'], $_POST['item_price'])) {
    ...
    $quantity = isset($_POST['item_quantity']) && is_numeric($_POST['item_quantity']) && $_POST['item_quantity'] > 0
                ...
                : 1; // Default to 1 if the quantity is invalid or missing
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update'])) {
    $index = $_POST['update'];
    $change = isset($_POST['change']) ? (int)$_POST['change'] : 0; // Amount to change the quantity by

    if (isset($_SESSION['cart'][$index])) {
        // Ensure the item has a quantity key
        if (!isset($_SESSION['cart'][$index]['quantity'])) {
            $_SESSION['cart'][$index]['quantity'] = 0; // Default to 0 if it's missing
        }
    ...
    <tbody>
      ...
      if (empty($_SESSION['cart'])) {
          ...
          foreach ($_SESSION['cart'] as $index => $item) {
              // Ensure 'quantity' is set to avoid undefined index error
