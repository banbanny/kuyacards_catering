...
if (isset($_POST["next"])) {
    ...
    $time = $_POST["time"];
    $refNo = $_POST["ref_no"];
    $paymentMode = $_POST["payment_mode"]; // Capture payment method
    ...
    if ($numRows > 0) {
        ...
        if ($stmt1->execute()) {
            ...
            // Insert into billing table with payment mode
            $sqlInsertBilling = "INSERT INTO billing (bookingID, status, amount, paymentMode) VALUES (?, 'Pending', '1', ?)";
            $stmt2 = $conn->prepare($sqlInsertBilling);
            $stmt2->bind_param("is", $bookingID, $paymentMode);
            $stmt2->execute();
            $stmt2->close();
