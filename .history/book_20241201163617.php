...
function generatePdf($db, $bookingID)
{
    // Retrieve the data for the invoice using the provided bookingID
    $sql = "SELECT c.date_of_delivery, c.time_of_delivery
            FROM customer_detail c
            WHERE c.id = :bookingID";
    $stmt = $db->prepare($sql);
    $stmt->bindParam(':bookingID', $bookingID);
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);

    $date = $result['date_of_delivery'];
    $time = $result['time_of_delivery'];
    ...
    // Clean the output buffer before generating the PDF to avoid the "TCPDF ERROR"
    ob_end_clean(); // Clean the buffer
    $pdf->Output($filename, 'D'); // Force download the generated PDF
