...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['login'])) {
        // Login Process
        $email = htmlspecialchars(strip_tags($_POST['email']));  // Sanitize email input
        $password = $_POST['password'];  // Password input is kept raw for comparison
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            // Debugging: Show entered password and hash stored in the database (only for debugging)
            // Remove this part in production for security reasons
            erro '<pre>';
            echo 'Entered Password: ' . htmlspecialchars($password) . '<br>';
            echo 'Stored Hash: ' . htmlspecialchars($user['password']) . '<br>';
            echo '</pre>';
            
            $login_error = "Invalid Email or Password!";  // Error message for invalid login
        }
    } 
