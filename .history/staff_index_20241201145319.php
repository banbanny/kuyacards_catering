...
<!-- Orders Table -->
  ...
  <table class="table table-bordered table-striped">
    ...
    <script>
  // Function to promote a customer to admin
  function promoteToAdmin(customerId) {
    if (confirm("Are you sure you want to promote this customer to admin?")) {
      // Send an AJAX request to promote the customer
      fetch("promote.php", {
        method: "POST",
        headers: {
        ...
        .then((data) => {
          if (data.success) {
            alert("Customer promoted to admin successfully!");
            location.reload(); // Reload the page to reflect changes
          } else {
            alert("Failed to promote the customer. " + data.message);
          }
        })
        .catch((error) => {
