...
if (isset($_POST["next"])) {
    ...
    if (empty($name) || empty($contact) || empty($address) || empty($dateFormat) || empty($time) || empty($paymentMethod)) {
    ...
    if ($numRows > 0) {
        ...
        $sqlInsertBooking = "INSERT INTO bookingdates (name, contact, address, date, time, refNo, paymentMethod) 
                             VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt1 = $conn->prepare($sqlInsertBooking);
        $stmt1->bind_param("sssssss", $name, $contact, $address, $dateFormat, $time, $refNo, $paymentMethod);
...
function generatePdf($conn, $bookingID)
{
    ...
    $sql = "SELECT b.bookingID, b.name, b.date, b.time
            FROM bookingdates b
            WHERE b.bookingID = ?";
