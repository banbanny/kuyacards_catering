...
<html lang="en">
  ...
  <table class="table table-bordered table-striped">
            ...
            <tbody>
              ...
              $stmt = $db->prepare("SELCT * FROM users WHERE role = 'admin'");
              $stmt->execute();

              $staff_result->fetchAll(PDO::FETCH_ASSOC);

              foreach($staff_result as $sresult)
               echo   '<tr>
                        <td>'.$sresult['id'].'</td>
                        <td>'.$sresult['full_name'].'</td>
                        <td>'.$sresult['role'].'</td>
                        <td>'.$sresult['contact_number'].'</td>
                        <td>
