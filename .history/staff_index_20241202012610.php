<?php
    ...
    <?php 
    include("php/catering_db.php");  // Include database connection

    // Prepare and execute the query to fetch customer details
    $stmt = $db->prepare("SELECT id, email, password, full_name, address, contact_number, role FROM customers");
    $stmt->execute();
          ...
          foreach ($customer_results as $customer) {
            echo '<tr>
                    ...
                    <td>
                      ...
                      <button class="btn btn-success btn-sm copy-btn" data-id="' . $customer['id'] . '">Copy</button>
                    </td>
                  </tr>';
          }
        ?>
